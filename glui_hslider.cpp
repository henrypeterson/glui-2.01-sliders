...
static inline int mround( float );
...
static inline int mround( float x )
{
   if( x >= 0. )
      return (int)( x + .5 );
...
// fprintf( stderr, "update_slider_val(): GLUI_LIVE_FLOAT = %d\n", GLUI_LIVE_FLOAT );
   ...
   if( button_is_resizable )
   {
      ...
      if( live_type == GLUI_LIVE_FLOAT )
         ...
         ip[1] = mround( button_right_edge );
   }

   if( live_type == GLUI_LIVE_FLOAT )
      fp[0] = button_left_edge;
   else
      ip[0] = mround( button_left_edge );
...
int GLUI_HSlider::redge_to_pixel( float val )
{
   ...
   return GLUI_HSLIDER_ARROW_WIDTH + 4 + mround( pct*pixels );
}
...
int GLUI_HSlider::ledge_to_pixel( float val )
{
   ...
   pixels = w - 2*GLUI_HSLIDER_ARROW_WIDTH - 4;
   pct = (val - float_low) / (float_high - float_low);
   return GLUI_HSLIDER_ARROW_WIDTH + mround( pct*pixels );
}
